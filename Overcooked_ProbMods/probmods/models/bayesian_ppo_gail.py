...
- Standard discriminator for stability
- PPO with KL regularization toward optional BC anchor
"""
...
class BayesianPPOGAILConfig:
    ...
    seed: int = 0
    bc_anchor_path: str | None = None


class Policy(nn.Module):
    """Standard policy network for PPO (non-Bayesian for gradient compatibility)."""
...
class Discriminator(nn.Module):
    ...
    def forward(self, s: torch.Tensor, a: torch.Tensor):
        x = torch.cat([s, F.one_hot(a, self.action_dim).float()], dim=-1)
        return torch.sigmoid(self.net(x)).squeeze(-1)


class BayesianPPOGAILTrainer:
    def __init__(self, config: BayesianPPOGAILConfig):
        self.config = config
    ...
    def _setup_models(self):
        ...
        self.discriminator = Discriminator(self.state_dim, self.action_dim, self.config.disc_hidden_dim).to(self.device)
        self.disc_optimizer = torch.optim.Adam(self.discriminator.parameters(), lr=self.config.disc_lr)

        self.bc_anchor = None
        if self.config.bc_anchor_path and Path(self.config.bc_anchor_path).exists():
            try:
                checkpoint = torch.load(self.config.bc_anchor_path, map_location=self.device)
                if isinstance(checkpoint, dict) and "policy_state_dict" in checkpoint:
                    self.bc_anchor = checkpoint["policy_state_dict"]
            except Exception:
                self.bc_anchor = None
    ...
    def _update_discriminator(self, policy_states: torch.Tensor, policy_actions: torch.Tensor):
        ...
        exp_idx = torch.randint(0, len(self.expert_states), (bs,), device=self.device)
        pol_idx = torch.randint(0, len(policy_states), (bs,), device=self.device)
        expert_s = self.expert_states[exp_idx]
        expert_a = self.expert_actions[exp_idx]
        policy_s = policy_states[pol_idx]
        policy_a = policy_actions[pol_idx]
        logits_exp = self.discriminator(expert_s, expert_a)
        logits_pol = self.discriminator(policy_s, policy_a)
        loss = (
            -torch.log(logits_exp + 1e-8).mean() * 0.5
            - torch.log(1 - logits_pol + 1e-8).mean() * 0.5
        )
        self.disc_optimizer.zero_grad()
        loss.backward()
        self.disc_optimizer.step()
        with torch.no_grad():
            acc = 0.5 * (logits_exp > 0.5).float().mean().item() + 0.5 * ((logits_pol < 0.5).float().mean().item())
    ...
    def _save(self, step: int):
        save_dir = Path(self.config.results_dir) / "bayesian_ppo_gail" / self.config.layout_name
...
def train_bayesian_ppo_gail(layout: str, verbose: bool = True, **kwargs):
    cfg = BayesianPPOGAILConfig(layout_name=layout, verbose=verbose, **kwargs)
    trainer = BayesianPPOGAILTrainer(cfg)
